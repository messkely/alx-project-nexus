...
class Command(BaseCommand):
    help = 'Seed the database with sample data for the e-commerce application'
    ...
    def handle(self, *args, **options):
        ...
        for i, user in enumerate(users):
            if not Address.objects.filter(user=user).exists() and i < len(addresses_data):
                ...
                self.stdout.write(self.style.SUCCESS(f"Created address for {user.email}"))
        ...
        for user in users[:3]:
            ...
            if created:
                ...
                self.stdout.write(self.style.SUCCESS(f"Created cart for {user.email}"))
        ...
        for user in users[:4]:
            if not Order.objects.filter(user=user).exists():
                ...
                self.stdout.write(self.style.SUCCESS(f"Created order for {user.email}"))
